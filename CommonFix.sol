...
function mulDivCeil(uint256 x, uint256 y, uint256 denominator) pure returns (uint256 result) {
    ...
    // => In the rest of this function, denominator >=1

    // Handle non-overflow cases, 256 by 256 division.
    if (prod1 == 0) { 
        unchecked {
            result = prod0 / denominator; // = floor(prod0 / denominator)
            if (result * denominator == prod0) // multiplication cannot overflow and is cheaper then computing MULMOD opcode 
            { // here denominator divides prod0
                return result;
            } 
            else 
            { // here denominator does not divides prod0. In particular we must have denominator >= 2. We want to return ceil(prod0 / denominator), so we must add one.
                return result + 1; // result <=floor(MAX_UINT256/denominator)<==floor(MAX_UINT256/2)<MAX_UINT256, so adding one cannot cause overflow
            }
        }
    }
...
/// @notice Calculates floor(x*y÷denominator) with full precision.
///
/// @dev An extension of `mulDivFloor` for signed numbers. Works by computing the signs and the absolute values separately.
///
/// Requirements:
...
function mulDivFloorSigned(int256 x, int256 y, int256 denominator) pure returns (int256) {
    ...
    if (sx ^ sy ^ sd == 0)
    {
        rAbs = mulDivCeil(absX, absY, absD); // Compute the absolute value of ceil(x*y)÷denominator.
        if (rAbs > uint256(type(int256).max)) {  // The result must fit within int256.
    ...
    {
        rAbs = mulDivFloor(absX, absY, absD); // Compute the absolute value of floor(x*y)÷denominator.
        if (rAbs > uint256(type(int256).max)) {  // The result must fit within int256.
            revert PRBMath_MulDivSigned_Overflow(x, y);
        }
        unchecked{
